...
const QuizNinja: React.FC<Props> = ({ userName = '', onComplete, onExit, tanakaSpeak }) => {
  ...
  const start = () => { setState('playing'); play('start'); speakT("Prenez votre temps pour bien réfléchir !"); };
  ...
  const next = () => {
    if (idx + 1 >= QUESTIONS.length) {
      ...
      if (correct >= 4) { playSuccess('high'); speakT(`Bravo ${userName} ! Votre culture est solide !`); }
      else { play('end'); speakT("Vous apprenez la Voie, continue !"); }
  ...
  return (
    <div className="relative min-h-[400px]" data-testid="quiz-aikidoka-game">
